...
go get -d github.com/dydysy/protoc-gen-validate
...
protoc \
  -I . \
  -I ${GOPATH}/src \
  -I ${GOPATH}/src/github.com/dydysy/protoc-gen-validate \
  --go_out=":../generated" \
...
PGV comes with a [Dockerfile](/Dockerfile) for consistent development tooling and CI. The main entrypoint is `make` with `quick` as the default target. This repo should be volumed into `/go/src/github.com/dydysy/protoc-gen-validate` for the proper behavior.
...
docker run --rm \
  -v $(PWD):/go/src/github.com/dydysy/protoc-gen-validate \
